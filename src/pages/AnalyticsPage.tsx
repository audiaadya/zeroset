...
export default function AnalyticsPage({ navigate }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const [{ data: summary }, { data: { session } }] = await Promise.all([
        fetchAnalyticsSummary().then((value) => ({ data: value })),
      ...
      if (session) {
        try {
          const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/host-analytics`, {
            headers: { Authorization: `Bearer ${session.access_token}` },
          });
          if (response.ok) authJson = await response.json();
        } catch {
          authJson = {};
        }
      }
      ...
      setData(summary ? {
        ...summary,
        totalAuthUsers: authJson.totalAuthUsers ?? users.length,
        verifiedUsers: authJson.verifiedUsers ?? users.filter((u) => Boolean(u.email_confirmed_at)).length,
        unverifiedUsers: authJson.unverifiedUsers ?? users.filter((u) => !u.email_confirmed_at).length,
        authUsers: users,
      } : null);
      setLoadingData(false);
    })();
  ...
  const profileUserIds = new Set(data.users.map((profile) => profile.user_id));
  const users = [
    ...
    ...authUsers
      .filter((auth) => !profileUserIds.has(auth.id))
      .map((auth) => ({
        profile: {
          id: auth.id,
          user_id: auth.id,
          display_name: auth.user_metadata?.display_name ?? null,
          email: auth.email,
          xp: 0,
          level: 1,
          streak: 0,
          ...
          last_active: auth.last_sign_in_at ?? auth.created_at,
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <StatTile icon={<Flame className="h-5 w-5" />} label="Visits (30d)" value={data.visits30d} accent="text-cream-200" />
          ...
          <section className="rounded-xl border border-ink-700 bg-ink-850/50 p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div className="rounded-lg border border-ink-700 bg-ink-900 p-4">
                ...
                <div className="mt-1 font-serif text-2xl text-cream-200">{data.unverifiedUsers || authUsers.filter(u => !u.email_confirmed_at).length}</div>
              </div>
            </div>
          ...
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatTile icon={<Zap className="h-5 w-5" />} label="Sudden Death" value={data.suddenDeathAttempts} accent="text-cream-200" />
            <StatTile icon={<Swords className="h-5 w-5" />} label="Duels" value={data.duelsOpen + data.duelsCompleted} accent="text-cream-200" />
          </section>
      ...
      {activeTab === 'users' && (
        <div className="space-y-6">
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatTile icon={<Flame className="h-5 w-5" />} label="New (30d)" value={data.newUsers30d} accent="text-cream-200" />
                        ...
                        <div className="bg-ink-900/50 p-4">
                          ...
                          {activeUserTab === 'summary' ? (
                            <div className="grid gap-4 text-sm sm:grid-cols-4">
                              ...
                              <div className="rounded-lg border border-ink-700 bg-ink-850 p-3"><div className="text-xs text-ink-400">Sudden Death</div><div className="font-mono text-lg text-ink-100">{userStats.sudden_death_attempts}</div><div className="text-xs text-cream-200">{userStats.sudden_death_correct} correct</div></div>
                              <div className="rounded-lg border border-ink-700 bg-ink-850 p-3"><div className="text-xs text-ink-400">Bounties</div><div className="font-mono text-lg text-ink-100">{userStats.bounties_posted}</div><div className="text-xs text-cream-200">{userStats.bounties_solved} solved</div></div>
                ...
                {data.streakDistribution.map((s) => {
                  ...
                  return (
                    <div key={s.streak} className="flex items-center gap-4">
                      ...
                      <div className="h-3 flex-1 overflow-hidden rounded-full bg-ink-900"><div className="h-full rounded-full bg-accent-400/60" style={{ width: `${(s.count / max) * 100}%` }} /></div>
      ...
      {activeTab === 'games' && (
        <div className="space-y-6">
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatTile icon={<Zap className="h-5 w-5" />} label="Sudden Death" value={data.suddenDeathAttempts} accent="text-cream-200" sublabel={`${data.suddenDeathCorrect} correct`} />
            <StatTile icon={<Bug className="h-5 w-5" />} label="Bounties Open" value={data.bountyOpen} accent="text-cream-200" sublabel={`${data.bountySolved} solved`} />
            ...
            <div className="divide-y divide-ink-700">
              {data.gameStats.slice(0, 20).map((g, idx) => (
                <div key={g.user_id} className="flex items-center justify-between p-4">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-8 w-8 items-center justify-center rounded-full font-mono text-sm font-semibold ${idx === 0 ? 'bg-amber-400/20 text-cream-200' : idx === 1 ? 'bg-gray-400/20 text-gray-300' : idx === 2 ? 'bg-orange-400/20 text-cream-200' : 'bg-ink-700 text-ink-400'}`}>{idx === 0 ? <Crown className="h-4 w-4" /> : idx + 1}</div>
                  ...
                  <div className="flex items-center gap-6 text-xs font-mono">
                    ...
                    <div className="text-center"><div className="text-cream-200">{g.sudden_death_attempts}</div><div className="text-ink-500">SD</div></div>
                    <div className="text-center"><div className="text-cream-200">{g.duels_won}</div><div className="text-ink-500">duels</div></div>
                  </div>
                </div>
              ))}
            </div>
