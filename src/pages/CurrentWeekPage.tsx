...
import { ArrowRight, Loader2, Sparkles, Zap, AlertCircle, CheckCircle2, XCircle, Lock, Heart, Info } from 'lucide-react';
        ...
        <div className="sm:pl-4">
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 font-handwritten text-sm text-ink-700">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="relative">
                <button
                  onMouseEnter={() => setShowSdInfo(true)}
                  onMouseLeave={() => setShowSdInfo(false)}
                  onFocus={() => setShowSdInfo(true)}
                  onBlur={() => setShowSdInfo(false)}
                  className="flex h-5 w-5 items-center justify-center rounded-full border border-ink-400 text-ink-600 hover:border-accent-500 hover:text-accent-700"
                  aria-label="Sudden Death info"
                >
                  <Info className="h-3 w-3" />
                </button>
                {showSdInfo && (
                  <div className="absolute right-0 top-7 z-20 w-64 border-2 border-ink-600 bg-cream-100 p-3 shadow-panel" style={{ borderRadius: '12px 4px 10px 4px / 4px 10px 4px 12px' }}>
                    <p className="font-handwritten text-xs leading-relaxed text-ink-800">
                      Get a question right and you get twice the XP. Get a question wrong, you can't submit another solution for the rest of the week.
                    </p>
                  </div>
                )}
              </div>
