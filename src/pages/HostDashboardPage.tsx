...
export default function HostDashboardPage({ navigate }: Props) {
  ...
  const [tab, setTab] = useState<'sets' | 'community' | 'analytics' | 'emails' | 'partners' | 'team'>('sets');
  ...
  useEffect(() => {
    ...
    if (tab === 'community' && communitySets.length === 0) {
      void loadCommunity();
    }
    if (tab === 'partners') {
      void loadPartners();
  ...
  const [communitySets, setCommunitySets] = useState<{ ws: DbWeekSet; problems: DbProblem[] }[]>([]);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
      <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        {tab === 'community' && (
          <button
            ...
            className="focus-ring flex items-center gap-1.5 rounded-md border border-accent-400 bg-accent-400/15 px-4 py-2 text-sm font-semibold text-accent-200 transition hover:bg-accent-400/25"
          >
            <Plus className="h-4 w-4" /> Create community set
          </button>
        )}
        {tab === 'partners' && (
          <button
      ...
      <div className="mb-6 flex gap-1 border-b border-ink-700">
        ...
        >
          <Users className="h-4 w-4" /> My Community Sets
        </button>
        <button
      ...
      {tab === 'community' && (
        <>
          {loadingCommunity ? (
            <div className="flex items-center gap-2 text-sm text-ink-400">
              <Loader2 className="h-4 w-4 animate-spin" /> Loading your community sets…
            </div>
          ) : communitySets.length === 0 ? (
            <div className="rounded-lg border border-ink-700 bg-ink-850/40 p-8 text-center">
              <Users className="mx-auto h-8 w-8 text-ink-500" />
              <p className="mt-3 text-sm text-ink-400">
                You don't have any community sets yet. Click "Create community set" to start one.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {communitySets.map(({ ws, problems }) => (
                <CommunitySetRow
                  key={ws.id}
                  ws={ws}
                  problemCount={problems.length}
                  onEdit={() => navigate(`/community/${ws.id}`)}
      ...
      {tab === 'analytics' && (
        <AnalyticsTab navigate={navigate} />
      )}
...
function CommunitySetRow({
  ...
  const remove = async () => {
    if (!confirm('Delete this community set? This cannot be undone.')) return;
    setBusy(true);
    await supabase.from('problems').delete().eq('set_id', ws.id);
    await supabase.from('week_sets').delete().eq('id', ws.id);
    setBusy(false);
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-lg border border-ink-700 bg-ink-850/50 p-4">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <h3 className="truncate font-serif text-base text-ink-50">{ws.title || 'Untitled set'}</h3>
          <span
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-ink-400">
          <span>{problemCount} problems</span>
          {ws.umbrella && <span>· {ws.umbrella}</span>}
          {ws.publish_at && (
            <span>· publishes {new Date(ws.publish_at).toLocaleDateString()}</span>
          )}
        </div>
      </div>
      <div className="flex shrink-0 items-center gap-2">
        <button
          ...
          disabled={busy}
          className="flex items-center gap-1.5 rounded-md border border-ink-700 px-3 py-1.5 text-xs text-ink-200 hover:border-accent-400/40 hover:text-accent-200 disabled:opacity-50"
        >
          {isPublished ? 'Unpublish' : 'Publish'}
        </button>
        <button
          onClick={onEdit}
          className="flex items-center gap-1.5 rounded-md border border-accent-400 bg-accent-400/15 px-3 py-1.5 text-xs font-semibold text-accent-200 hover:bg-accent-400/25"
        >
          <Edit3 className="h-3.5 w-3.5" /> Edit
        </button>
        <button
          onClick={remove}
          disabled={busy}
          className="flex items-center gap-1.5 rounded-md border border-red-500/40 bg-red-500/10 px-3 py-1.5 text-xs text-red-300 hover:bg-red-500/20 disabled:opacity-50"
        >
          <Trash2 className="h-3.5 w-3.5" />
        </button>
      </div>
...
function AnalyticsTab({ navigate }: { navigate: (to: string) => void }) {
  const { isHost, configured } = useAuth();
  const [summary, setSummary] = useState<AnalyticsSummary | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!configured || !isHost) {
      setLoading(false);
      return;
    }
    (async () => {
  ...
  if (!summary) {
    return (
      <div className="rounded-lg border border-ink-700 bg-ink-850/40 p-8 text-center">
        ...
        <p className="mt-3 text-sm text-ink-400">No analytics data yet.</p>
      </div>
    );
  }

  const tiles = [
    { label: 'Total visits', value: summary.totalVisits, icon: <Eye className="h-4 w-4" />, accent: 'text-sky-300' },
    { label: 'Unique visitors', value: summary.uniqueVisitors, icon: <Users className="h-4 w-4" />, accent: 'text-accent-300' },
    { label: 'Sign-ups', value: summary.totalUsers, icon: <Mail className="h-4 w-4" />, accent: 'text-violet-300' },
    { label: 'Solutions submitted', value: summary.totalSolutions, icon: <Send className="h-4 w-4" />, accent: 'text-emerald-300' },
    { label: 'Correct solutions', value: summary.correctSolutions, icon: <Check className="h-4 w-4" />, accent: 'text-accent-300' },
    { label: 'Weekly active solvers', value: summary.weeklyActiveSolvers, icon: <Clock className="h-4 w-4" />, accent: 'text-orange-300' },
    { label: 'New users (7d)', value: summary.newUsers7d, icon: <Users className="h-4 w-4" />, accent: 'text-emerald-300' },
    { label: 'Retention rate', value: `${summary.retentionRate}%`, icon: <BarChart3 className="h-4 w-4" />, accent: 'text-amber-300' },
  ];
  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {tiles.map((t) => (
          <div key={t.label} className="rounded-xl border border-ink-700 bg-ink-850/50 p-5">
            <div className={`mb-2 flex h-9 w-9 items-center justify-center rounded-md border border-ink-700 bg-ink-900 ${t.accent}`}>
              {t.icon}
            </div>
            <div className="font-serif text-3xl text-ink-50">{typeof t.value === 'number' ? t.value.toLocaleString() : t.value}</div>
            <div className="mt-1 text-xs text-ink-400">{t.label}</div>
          </div>
        ))}
      </section>

      {summary.dailyVisits.some((d) => d.count > 0) && (
        <section className="rounded-xl border border-ink-700 bg-ink-850/50 p-6">
          ...
          <div className="flex h-40 items-end gap-1.5">
            {summary.dailyVisits.map((d) => {
              const max = Math.max(1, ...summary.dailyVisits.map((x) => x.count));
              return (
