...
export default function HostDashboardPage({ navigate }: Props) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
      ...
      {error && (
        <div className="mb-6 flex items-start gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-cream-200">
                ...
                <div key={p.id} className="group relative overflow-hidden rounded-xl border border-ink-700 bg-ink-850/50 transition hover:border-accent-400/30">
                  ...
                  <div className="absolute right-2 top-2 flex gap-1 opacity-0 transition group-hover:opacity-100">
                    ...
                    <button onClick={async () => {
                      ...
                      void loadPartners();
                    }} className="rounded-md border border-red-500/40 bg-ink-900/80 p-1.5 text-cream-200 hover:bg-red-500/20">
    ...
    <div className="flex flex-col gap-3 rounded-lg border border-ink-700 bg-ink-850/50 p-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={remove}
          disabled={deleting}
          className="focus-ring rounded-md border border-red-500/30 p-1.5 text-cream-200 transition hover:bg-red-500/10 disabled:opacity-50"
          title="Delete set"
        >
      ...
      <div className="relative my-4 flex max-h-[calc(100vh-2rem)] w-full max-w-3xl animate-fade-in flex-col overflow-hidden rounded-xl border border-ink-700 bg-ink-900 shadow-panel">
        ...
        <div className="flex-1 space-y-5 overflow-y-auto overscroll-contain p-5 md:pr-80">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <label className="block">
              <span className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-ink-300">Week #</span>
              <input
                type="number"
                value={weekNumber}
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4">
      ...
      <div className="relative my-4 flex max-h-[calc(100vh-2rem)] w-full max-w-lg animate-fade-in flex-col overflow-hidden rounded-xl border border-ink-700 bg-ink-900 shadow-panel">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto overscroll-contain p-5">
          ...
          <div>
            ...
            {imageUrl ? (
              <div className="group relative overflow-hidden rounded-md border border-ink-700">
                ...
                <button onClick={() => setImageUrl('')} className="absolute right-2 top-2 rounded-md border border-red-500/40 bg-ink-900/80 p-1.5 text-cream-200 hover:bg-red-500/20">
                  <Trash2 className="h-3.5 w-3.5" />
                </button>
              </div>
            ) : (
          ...
          {error && (
            <div className="flex items-start gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-cream-200">
...
function MetricTile({ icon, label, value, accent }: { icon: ReactNode; label: string; value: number | string; accent: string }) {
  return (
    <div className="rounded-xl border border-ink-700 bg-ink-850/50 p-5">
      ...
      <div className={`mt-2 font-serif text-2xl ${accent}`}>{value}</div>
    </div>
  );
}
...
function AnalyticsTab({ metrics, loading, source, navigate }: { metrics: HostDashboardMetrics | null; loading: boolean; source: 'rpc' | 'local' | null; navigate: (to: string) => void }) {
  ...
  if (!metrics) {
    return (
      <div className="rounded-lg border border-ink-700 bg-ink-850/40 p-8 text-center">
        ...
        <p className="mt-3 text-sm text-ink-400">No analytics data available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {source && (
          <span className={`rounded-full px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider ${
            source === 'rpc'
              ? 'border border-accent-400/40 bg-accent-400/10 text-accent-300'
              : 'border border-cream-200/30 bg-cream-200/10 text-cream-200'
          }`}>
      ...
      <section>
        <h2 className="mb-3 font-serif text-lg text-ink-50">User Conversion</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <MetricTile icon={<Users className="h-5 w-5" />} label="Total Registered Users" value={m.total_signups} accent="text-accent-300" />
          <MetricTile icon={<TrendingUp className="h-5 w-5" />} label="New Users (7d)" value={m.new_users_7d} accent="text-emerald-300" />
          <MetricTile icon={<Flame className="h-5 w-5" />} label="New Users (30d)" value={m.new_users_30d} accent="text-cream-200" />
          <MetricTile icon={<Zap className="h-5 w-5" />} label="Active Solvers (7d)" value={m.weekly_active_solvers} accent="text-sky-300" />
        </div>
      </section>

      <section>
        <h2 className="mb-3 font-serif text-lg text-ink-50">Engagement Metrics</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <MetricTile icon={<BarChart3 className="h-5 w-5" />} label="Solutions Submitted" value={m.total_solution_submissions} accent="text-violet-300" />
          <MetricTile icon={<CheckCircle2 className="h-5 w-5" />} label="Correct Solutions" value={m.correct_solutions} accent="text-accent-300" />
          <MetricTile icon={<CheckCircle2 className="h-5 w-5" />} label="Verification Rate" value={`${verificationRate}%`} accent="text-emerald-300" />
          <MetricTile icon={<Zap className="h-5 w-5" />} label="Sudden Death Failures" value={m.sudden_death_failures} accent="text-red-300" />
        </div>
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          <MetricTile icon={<Zap className="h-5 w-5" />} label="Sudden Death Attempts" value={m.sudden_death_attempts} accent="text-cream-200" />
          <MetricTile icon={<Swords className="h-5 w-5" />} label="Bounties Open" value={m.bounty_open} accent="text-sky-300" />
          <MetricTile icon={<Swords className="h-5 w-5" />} label="Bounties Solved" value={m.bounty_solved} accent="text-accent-300" />
        </div>
      </section>

      <section>
        <h2 className="mb-3 font-serif text-lg text-ink-50">Site Traffic</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <MetricTile icon={<Eye className="h-5 w-5" />} label="Total Pageviews" value={m.total_pageviews} accent="text-sky-300" />
          <MetricTile icon={<TrendingUp className="h-5 w-5" />} label="Pageviews (7d)" value={m.pageviews_7d} accent="text-emerald-300" />
          <MetricTile icon={<Users className="h-5 w-5" />} label="Unique Visitors" value={m.unique_active_visitors} accent="text-accent-300" />
          <MetricTile icon={<MousePointerClick className="h-5 w-5" />} label="CTA Clicks" value={m.cta_clicks} accent="text-violet-300" />
        </div>
      </section>

      <section className="rounded-xl border border-ink-700 bg-ink-850/50 p-6">
        <h2 className="mb-4 font-serif text-xl text-ink-50">Pageviews — last 14 days</h2>
        {m.daily_pageviews.every((d) => d.count === 0) ? (
          <p className="text-sm text-ink-400">No pageview data yet. Events are being collected.</p>
        ) : (
          <div className="flex h-40 items-end gap-1.5">
            {m.daily_pageviews.map((d) => (
              <div key={d.day} className="flex flex-1 flex-col items-center gap-1">
                <div
                  className="w-full rounded-t bg-gradient-to-t from-accent-500/40 to-accent-400 transition-all"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
                ...
                return (
                  <div key={p.path} className="flex items-center gap-3">
                    ...
                    <div className="h-2 flex-1 overflow-hidden rounded-full bg-ink-900">
                      <div className="h-full bg-accent-400/60" style={{ width: `${(p.count / max) * 100}%` }} />
                    </div>
        ...
        <section className="rounded-xl border border-ink-700 bg-ink-850/50 p-6">
          <h2 className="mb-4 font-serif text-xl text-ink-50">CTA Click Breakdown</h2>
          {m.cta_breakdown.length === 0 ? (
            <p className="text-sm text-ink-400">No CTA clicks recorded yet.</p>
          ) : (
            <div className="space-y-2">
              {m.cta_breakdown.map((c) => {
                ...
                return (
                  <div key={c.cta_id} className="flex items-center gap-3">
                    <span className="w-32 truncate font-mono text-xs text-ink-300">{c.cta_id}</span>
                    <div className="h-2 flex-1 overflow-hidden rounded-full bg-ink-900">
                      <div className="h-full bg-violet-400/60" style={{ width: `${(c.count / max) * 100}%` }} />
                    </div>
                    <span className="w-12 text-right font-mono text-xs text-ink-400">{c.count}</span>
                  </div>
                );
              })}
            </div>
      ...
      <div className="flex justify-end">
        <button
          ...
          className="flex items-center gap-1.5 rounded-md border border-ink-700 px-4 py-2 text-sm text-ink-300 hover:border-accent-400/40 hover:text-accent-200"
        >
