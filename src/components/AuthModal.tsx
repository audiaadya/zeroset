  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md animate-fade-in rounded-xl border border-ink-700 bg-ink-900 shadow-panel">
        ...
        <form onSubmit={submit} className="space-y-3 p-5 pt-0">
          {mode === 'signup' && (
            <Field
          ...
          {mode === 'signup' && (
            <label className="flex items-start gap-2.5 rounded-md border border-ink-700 bg-ink-850 px-3 py-2.5">
              <input
                type="checkbox"
                checked={emailConsent}
                onChange={(e) => setEmailConsent(e.target.checked)}
                className="mt-0.5 h-4 w-4 rounded border-ink-600 bg-ink-900 text-accent-500 focus:ring-accent-400"
              />
