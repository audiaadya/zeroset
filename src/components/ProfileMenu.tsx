...
import { ChevronDown, User, BarChart3, Settings, LogOut, Crown, Layers, Users, BarChart3 as BarChart3Icon, LifeBuoy, Megaphone, Edit3, FileText, Shield } from 'lucide-react';
...
export default function ProfileMenu({ navigate, onSignIn, onSignUp }: Props) {
  ...
  return (
    <div className="relative hidden md:block" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="focus-ring flex items-center gap-2 rounded-md border border-ink-700 bg-ink-850 px-2.5 py-1.5 text-xs text-white transition hover:border-ink-600"
      >
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-60 animate-fade-in overflow-hidden rounded-lg border border-ink-700 bg-ink-900 shadow-panel">
          <div className="border-b border-ink-700 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                ...
                <div className="truncate text-[10px] text-cream-300">{user.email}</div>
              </div>
            </div>
            {isHost && (
          ...
          <div className="p-1.5">
            ...
            <MenuItem icon={<Layers className="h-4 w-4" />} label="My weekly sets" onClick={() => { setOpen(false); navigate('/my-sets'); }} />
            {isHost && (
              <MenuItem icon={<Crown className="h-4 w-4" />} label="Host dashboard" onClick={() => { setOpen(false); navigate('/host'); }} />
            )}
  ...
  return (
    <button
      ...
      className={`focus-ring flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-left text-sm transition ${
        danger ? 'text-cream-200 hover:bg-red-500/10 hover:text-red-300' : 'text-white hover:bg-ink-800 hover:text-white'
      }`}
    >
