...
export async function listTeamMembers(): Promise<TeamMember[]> {
  ...
  const tmByEmail = new Map<string, TeamMember>();
  for (const row of (tm as TeamMember[]) ?? []) {
    if (row.user_id) tmByUserId.set(row.user_id, row);
    if (row.email) tmByEmail.set(row.email.toLowerCase(), row);
  }
  ...
  for (const p of (profs ?? []) as Array<{
    ...
    const existing = tmByUserId.get(p.user_id) ?? (p.email ? tmByEmail.get(p.email.toLowerCase()) : undefined);
    const lb = {
      xp: p.xp ?? 0,
      level: p.level ?? 1,
      streak: p.streak ?? 0,
  ...
  // Include team_members rows that have no matching profile (pre-seeded staff
  // who haven't registered yet, e.g. an invited community manager).
  for (const row of (tm as TeamMember[]) ?? []) {
    const hasProfile = row.user_id && seenUserIds.has(row.user_id);
    const matchedByEmail = !hasProfile && row.email && profs?.some(
      (p) => p.email?.toLowerCase() === row.email!.toLowerCase()
    );
    if (!hasProfile && !matchedByEmail) {
      out.push({
        ...row,
        xp: 0,
        level: 1,
